            ...
            <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
                ...
                <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="border-b border-gray-200 px-5 py-4 dark:border-gray-800 sm:px-6">
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Edit Subject</h3>
                    </div>

                    <form action="{{ route('admin.subjects.update', $subject) }}" method="POST" class="p-5 sm:p-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                    ...
                                    @forelse($classes as $class)
                                        <label class="flex items-center gap-2">
                                            <input type="checkbox" name="classes[]" value="{{ $class->id }}"
                                                   {{ in_array($class->id, old('classes', $subject->schoolClasses->pluck('id')->toArray())) ? 'checked' : '' }}
                                                   class="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="3"
                                          class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white">{{ old('description', $subject->description) }}</textarea>
                            </div>
                        </div>
