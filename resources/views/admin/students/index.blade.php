                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                                        <td class="px-5 py-4">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="font-medium text-gray-800 dark:text-white">{{ $student->first_name }} {{ $student->last_name }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-5 py-4 text-gray-600 dark:text-gray-400">{{ $student->email }}</td>
                                        <td class="px-5 py-4 text-gray-600 dark:text-gray-400">{{ $student->phone ?? '-' }}</td>
                                        <td class="px-5 py-4">
                                            <form action="{{ route('admin.students.change-role', $student) }}" method="POST" class="inline">
                                                @csrf
                                                @method('PATCH')
                                                <select name="role" onchange="this.form.submit()"
                                                        class="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                                        ...
                                        <td class="px-5 py-4 text-gray-600 dark:text-gray-400">{{ $student->created_at->format('M d, Y') }}</td>
                                        <td class="px-5 py-4">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-5 py-8 text-center text-gray-500 dark:text-gray-400">
                                            No students found. <a href="{{ route('admin.students.create') }}" class="text-brand-500 hover:underline">Add one now</a>.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
